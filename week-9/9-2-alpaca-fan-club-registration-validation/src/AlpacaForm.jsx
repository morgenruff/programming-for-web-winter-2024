...
export function AlpacaForm () {
  ...
  function validateFirstName() {
    if (!formObject.firstName) { // if first name isn't populated, return required error message
      setErrorObject({
        ...errorObject,
  ...
  function validateLastName() {
      if (!formObject.lastName) { // if last name isn't populated, return required error message
        setErrorObject({
          ...errorObject,
  ...
  function handleSubmit (event) { // handle user form submission
    event.preventDefault(); // prevent page refresh on change
    validateFirstName(); // run validation on first name
    validateLastName(); // run validation on last name
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        ...
        {/* for the button, disable click if validation detects errors, otherwise allow submit */}
        <button type='submit' disabled={errorObject.firstName || errorObject.country}>&#129433; SIGN ME UP! &#129433;</button>
      </form>
    </>
  )
}